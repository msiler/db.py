...
def make_query_key(limit, unique):
    l = ["select"]
...
class TableMeta(object):
    def __init__(self, query_templates, name, cols, keys_per_column, con):
        ...
        cur.execute(query_templates['system']['foreign_keys_for_table'].format(table=self.name))
        for (column_name, foreign_table, foreign_column) in cur:
            col = col_map[column_name]
            foreign_key = Column(con, queries_templates, foreign_table, foreign_column, col.type, keys_per_column)
        ...
        cur.execute(query_templates['system']['ref_keys_for_table'].format(table=self.name))
        for (column_name, ref_table, ref_column) in cur:
            col = col_map[column_name]
            ref_key = Column(con, queries_templates, ref_table, ref_column, col.type, keys_per_column)
...
class Table(object):
    ...
    def __init__(self, con, query_templates, name, cols, keys_per_column):
        self._m = TableMeta(query_templates, name, cols, keys_per_column, con)
        self.name = name  # could be overwritten
        for col in cols:
    ...
    def __repr__(self):
        ...
        r = tbl.split('\n')[0]
        brk = "+" + "-"*(len(r)-2) + "+"
        title = "|" + self._m.name.center(len(r)-2) + "|"
        return brk + "\n" + title + "\n" + tbl

    def __str__(self):
        return "Table({0})<{1}>".format(self._m.name, self.__hash__())

    def _repr_html_(self):
        return self._tablify().get_html_string()

    def __call__(self, *args, limit=None, unique=False):
        if len(args) == 0:
            columns = "*"
        else:
            columns = ", ".join(args)
        query_key = make_query_key(limit, unique)
        q = self._m.query_templates['table'][query_key].format(columns=columns, limit=limit, table=self._m.name)
        return pd.io.sql.read_sql(q, self._m.con)

    def __len__(self):
        q = self._m.query_templates['table']['count'].format(table=self._m.name)
        return pd.io.sql.read_sql(q, self._m.con).values[0][0]
...
class TableSet(object):
    ...
    def _tablify(self):
        ...
        for table in self.tables:
            ...
            tbl.add_row([table._m.name, pretty_column_names])
        return tbl
...
class DB(object):
    ...
    def load_credentials(self, profile="default"):
        ...
        if os.path.exists(f):
            ...
            raw_creds = base64.decodebytes(raw_creds).decode('utf-8')
    ...
    def save_credentials(self, profile="default"):
        ...
        with open(dotfile, 'wb') as f:
            ...
            try:
                ...
                f.write(base64.encodebytes(bytes(data, 'utf-8')))
    ...
    def find_table(self, search):
        ...
        tables = []
        for table in self.tables:
            if glob.fnmatch.fnmatch(table._m.name, search):
                tables.append(table)
...
def list_profiles():
    ...
    for f in os.listdir(user):
        if f.startswith(".db.py_"):
            ...
            profile = json.loads(base64.decodebytes(open(profile, 'rb').read()).decode('UTF-8'))
